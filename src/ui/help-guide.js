...
const GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '事项可在原位置确认完成、暂停或按误登记移除；“查看停止项”可查看并恢复追踪，不会重读或调用模型。需要永久删除已停止的事项时，展开“查看停止项”，点击“批量管理”并选择事项，再点“批量永久删除”确认。删除包含该事项的全部时间历史，不能恢复；不会删除其他事项、摘要或千事，也不调用模型。取消编辑不保存，保存失败保留草稿供重试。停止只影响当前追踪，不删除旧批次、摘要、CSE或楼内历史回执。',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '楼内“事”页可展开“本轮千事进度”，查看本轮实际注入的千事内容；这里只展示本轮已经注入的材料。',
    ],
  },
  {
    title: '千事',
    paragraphs: [
      '“千事”按剧情时间展示已保存事件，可按事件、说明、人物、对象或时间搜索。展开事件可看完整说明和来源楼；同一天的事件会放在一起，同一事项也可展开查看完整经过。没有可靠剧情日期的事件会单独列出，不会按楼号猜日期。',
      '页顶显示千事整理覆盖情况。“补齐旧楼”会先列出待处理楼数、批次和预计摘要 API 调用次数；你确认“开始补齐”后才会调用模型。已完成批次会保留，可停止后再规划剩余部分。缺少唯一有效摘要的楼不会被这项操作处理。',
    ],
  },
  {
    title: '设置',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '要找旧摘要，可在“千结”页使用“搜索全部摘要”；找人物状态变化可在“双丝网”打开“分析记录”并搜索。它们会搜索当前聊天已有记录。查看当前聊天后台记录占用或清理不再使用的旧版本，打开“设置 → 记忆设置 → 存储管理”。先点“刷新统计”查看；“清理旧版本”会再次确认，只删除已不被当前数据引用的后台旧文件。',
    ],
    items: [
  ...
  },
  {
    title: '排障手册',
    paragraphs: [
      '大量楼摘要缺正文时，先检查“设置 → 通用设置 → 提示词与包裹符 → 包裹符”的“保留包裹符”。默认保留 content；如果正文都被别的成对标签包住，而标签名没加入保留名单，清洗后可能没有内容。普通额外标签名不会覆盖保留设置；若填过 [[...]] 这类字面规则，也检查它是否圈住正文。改正后回千结或记忆管理刷新状态，并按界面提示补齐。',
